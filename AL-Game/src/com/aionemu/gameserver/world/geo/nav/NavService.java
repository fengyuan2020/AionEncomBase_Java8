...
public final class NavService {
    ...
    // Cache the last known NavGeometry for each Creature.  Consider using a WeakHashMap
    // to avoid memory leaks if Creatures are frequently created and destroyed.
    //private final Map<Creature, NavGeometry> creatureNavTileCache = new WeakHashMap<>();  //Consider using a WeakHashMap or ConcurrentHashMap depending on the multithreading needs
    ...
    private float[][] attemptStraightLinePath(NavGeometry tile1, NavGeometry tile2, float x1, float y1, float z1, float x2, float y2, float z2) {
        ...
        int triCount = 0;
        while (triCount < GeoDataConfig.GEO_NAV_MAX_STRAIGHT_LINE_TRIANGLES && current != tile2 && current != null) {
            triCount++;
...
//		if (target.getLifeStats().isAlreadyDead()) return null;
    ...
    private float[][] navigateFromLocationToLocation(int worldId, NavGeometry tile, NavGeometry tile2, float x1, float y1, float z1, float x2, float y2, float z2) {
        ...
        if (boxed) {
            ...
            if (pathFromP == null) {
                NavHelper helper = null;
                try {
                    helper = NavHelper.borrowObject(tile, tile2, p[0], p[1], p[2], x2, y2, z2);
                    NavPathway[] pathway = helper.createPathway();
                    pathFromP = funnelPathway(pathway, tile2 != null, p[0], p[1], p[2], x2, y2, z2);
                } catch (Exception e) {
                    log.error("Error borrowing/creating NavHelper: ", e);
                    return null; // Or handle the error appropriately
                } finally {
                    NavHelper.returnObject(helper);
                }
            }

            path = new float[pathFromP.length + 1][];
            path[0] = p;
            System.arraycopy(pathFromP, 0, path, 1, pathFromP.length);
        } else {
            ...
            NavHelper helper = null;
            try {
                helper = NavHelper.borrowObject(tile, tile2, x1, y1, z1, x2, y2, z2);
                NavPathway[] pathway = helper.createPathway();
                path = funnelPathway(pathway, tile2 != null, x1, y1, z1, x2, y2, z2);
            } catch (Exception e) {
                log.error("Error borrowing/creating NavHelper: ", e);
                return null; // Or handle the error appropriately
            } finally {
    ...
    private static boolean areEqualPoints(float[] p1, float[] p2) {
        ...
        final float FLOAT_EQUALS_EPSILON = 0.0001f; // You can move this to a config or constant
        return Math.abs(p1[0] - p2[0]) < FLOAT_EQUALS_EPSILON &&
                Math.abs(p1[1] - p2[1]) < FLOAT_EQUALS_EPSILON &&
                Math.abs(p1[2] - p2[2]) < FLOAT_EQUALS_EPSILON;
